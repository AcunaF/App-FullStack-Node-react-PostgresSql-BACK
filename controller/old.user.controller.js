...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (!login) {
      return res.json({ message: "Case 1: user not found" });
    }

    // const passwordMatch = await bcrypt.compare(contrasena, login.contrasena);
    const passwordMatch = login.contrasena === contrasena;
    console.log("ulogin");

    if (passwordMatch) {
      // Si las contraseñas coinciden, puedes responder con un mensaje de éxito
      res.json({ message: "Inicio de sesión exitoso 2333" });
    } else {
    ...
    res.status(500).json({ error: "Error en el inicio de sesión bbbb" });
  }
});
